...
void TXshSoundTextColumn::createSoundTextLevel(int row,
  ...
  // Level managed by smart pointer; refcount handled via TXshCell
  TSmartPointerT<TXshSoundTextLevel> level = new TXshSoundTextLevel();
  ...
  for (int i = 0; i < textList.size(); i++, row++) {
    ...
    // "<none>": reuse previous cell, except for first entry
    if (str == "<none>") {
      if (i > 0) {
        setCell(row, cell);
        continue;
    ...
    TFrameId fid(i + 1);
    cell = TXshCell(level.getPointer(), fid);
    ...
    if (tagName == "cells") {
      while (is.openChild(tagName)) {
        if (tagName == "cell") {
          ...
          QString qs       = QString::fromStdString(rowRangeStr);
          QStringList rows = qs.split('-');
...
void TXshSoundTextColumn::saveData(TOStream &os) {
  ...
  if (getRange(r0, r1)) {
    ...
    for (int r = r0; r <= r1; r++) {
      ...
      if (r == r1 && !cell.isEmpty()) {
        int toR      = r;
        TFrameId fid = cell.m_frameId;

        if (fromR == toR) {
          os.child("cell") << toR << fid.getNumber()
