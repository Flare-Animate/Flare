...
public:
  PresetNamePopup(QWidget *parent = nullptr) : QDialog(parent) {
...
// Brush Preset Panel display prefs in user .env (TEnv), not the Windows
// registry.
TEnv::IntVar BrushPresetPanelShowBorders("BrushPresetPanelShowBorders", 0);
TEnv::IntVar BrushPresetPanelShowBackgrounds("BrushPresetPanelShowBackgrounds",
                                             1);
TEnv::IntVar BrushPresetPanelViewMode(
    "BrushPresetPanelViewMode", static_cast<int>(BrushPresetPanel::GridLarge));
...
BrushPresetItem::BrushPresetItem(const QString &presetName,
                                 const QString &toolType, bool isListMode,
                                 QWidget *parent)
    : QToolButton(parent)
  ...
  connect(this, &QPushButton::clicked,
          [this]() { emit presetSelected(m_presetName, m_toolType); });
...
void BrushPresetItem::updateScaledIcon() {
  ...
  // This optimization improves visual balance on screens < 4K (2.5K, 1080p,
  // 1050p)
  ...
  m_scaledIconCache = m_iconPixmap.scaled(
      scaledWidth, scaledHeight, Qt::KeepAspectRatio, Qt::SmoothTransformation);
}
...
void BrushPresetItem::paintEvent(QPaintEvent *event) {
  ...
  initStyleOption(&opt);
  const bool useThemeState = opt.state.testFlag(QStyle::State_MouseOver) ||
  ...
  if (m_hasIcon && !m_iconPixmap.isNull()) {
    ...
    if (m_isListMode) {
      ...
      if (!m_scaledIconCache.isNull()) {
        ...
        QRect textRect(
            iconX + m_scaledIconCache.width() + margin, margin,
        ...
        if (fm.horizontalAdvance(displayText) > textRect.width()) {
          displayText =
              fm.elidedText(displayText, Qt::ElideMiddle, textRect.width());
        }
      ...
      const int textHeightSmall =
          16;                    // Height reserved for text below (larger)
      ...
      if (!m_scaledIconCache.isNull()) {
        ...
        int availableHeight =
            height() - (2 * margin) - textHeightSmall - textMargin;
      ...
      QRect textRect(margin, textStartY, width() - (2 * margin),
                     textHeightSmall);
...
void BrushPresetPanel::initializeUI() {
  ...
  // Use native Flare icons matching ToolOptionBar design
  ...
  // Hamburger menu for display modes - use standard Flare menu icon
  ...
  setWindowTitle(tr("Brush Presets") +
                 tr(" [WIP]"));  // Temporarily add WIP label
  ...
  // Connect buttons
  connect(m_addPresetButton, SIGNAL(clicked()), this,
          SLOT(onAddPresetClicked()));
  ...
  for (int i = 0; i < props->getPropertyCount(); ++i) {
    ...
    if (prop->getName() == "Preset:") {
      ...
      if (presetProp) {
        ...
        } else if (toolType == "raster" || toolType == "mypaint" ||
                   toolType == "mypainttnz") {
          orderFileName = "brush_raster_order.txt";
        }

        if (!orderFileName.isEmpty()) {
          TFilePath orderFilePath =
              TEnv::getConfigDir() + orderFileName.toStdString();
          ...
          if (orderFile.exists() &&
            ...
            while (!in.atEnd()) {
              ...
              if (!presetName.isEmpty() && allPresets.contains(presetName) &&
                  !addedPresets.contains(presetName)) {
...
void BrushPresetPanel::refreshPresetList() {
  ...
  int optimalColumns =
      qMax(1, (panelWidth + spacing) / (itemSize.width() + spacing));
  ...
  for (const QString &presetName : presets) {
    BrushPresetItem *item = new BrushPresetItem(presetName, toolType,
                                                isListMode, m_presetContainer);
    ...
    connect(item, &BrushPresetItem::presetSelected, this,
            &BrushPresetPanel::onPresetItemClicked);
    connect(item, &BrushPresetItem::presetReordered, this,
            &BrushPresetPanel::reorderPreset);
  ...
  if (currentPresetName.isEmpty() ||
      currentPresetName == QString::fromStdWString(L"<custom>")) {
    m_removePresetButton->setEnabled(false);
  }
}
...
void BrushPresetPanel::applyPreset(const QString &presetName) {
  ...
  for (int i = 0; i < m_presetLayout->count(); ++i) {
    ...
    if (BrushPresetItem *presetItem =
            dynamic_cast<BrushPresetItem *>(item->widget())) {
      if (presetItem->getPresetName() == presetName) {
        presetItem->setChecked(
            true);  // QButtonGroup will uncheck others automatically
        break;
      }
    }
  }
  ...
  m_removePresetButton->setEnabled(!presetName.isEmpty() &&
                                   presetName !=
                                       QString::fromStdWString(L"<custom>"));
}
...
void BrushPresetPanel::addNewPreset() {
  ...
  QMessageBox::information(
      this, tr("Add Preset"),
      tr("Please use the [+] button in the Tool Options Bar to add new "
         "presets.\n\n"
...
void BrushPresetPanel::removeCurrentPreset() {
  ...
  int ret = QMessageBox::question(
      ...
      tr("Are you sure you want to remove the preset '%1'?")
          .arg(m_currentPreset),
  ...
  QMessageBox::information(
      this, tr("Remove Preset"),
      tr("Please use the [-] button in the Tool Options Bar to remove "
         "presets.\n\n"
...
void BrushPresetPanel::reorderPreset(const QString &fromPreset,
                                     const QString &toPreset) {
  ...
  if (m_currentToolType == "vector") {
    ...
    orderFileName = "brush_toonzraster_order.txt";
  } else if (m_currentToolType == "raster" || m_currentToolType == "mypaint" ||
...
void BrushPresetPanel::onToolSwitched() { refreshPresetList(); }
  ...
  for (int i = 0; i < props->getPropertyCount(); ++i) {
    ...
    if (prop->getName() == "Preset:") {
      ...
      if (presetProp) {
        ...
        // Always update visual selection (even if m_currentPreset hasn't
        // changed) because another panel may have changed the preset
        ...
        if (newPreset.isEmpty() ||
            newPreset == QString::fromStdWString(L"<custom>")) {
          m_removePresetButton->setEnabled(false);
        } else {
          m_removePresetButton->setEnabled(true);
        }
...
void BrushPresetPanel::onPresetItemClicked(const QString &presetName,
                                           const QString &toolType) {
  applyPreset(presetName);
}

void BrushPresetPanel::onAddPresetClicked() { addNewPreset(); }

void BrushPresetPanel::onRemovePresetClicked() { removeCurrentPreset(); }

void BrushPresetPanel::onRefreshClicked() { refreshPresetList(); }
...
void BrushPresetPanel::createViewModeMenu() {
  ...
  connect(m_viewModeButton, &QPushButton::clicked, [this]() {
    ...
    QPoint pos =
        m_viewModeButton->mapToGlobal(QPoint(0, m_viewModeButton->height()));
...
void BrushPresetPanel::addShowHideContextMenu(QMenu *menu) {
  ...
  connect(bordersAction, SIGNAL(triggered()), this,
  ...
  connect(backgroundsAction, SIGNAL(triggered()), this,
          SLOT(onShowHideActionTriggered()));
}
...
void BrushPresetPanel::updateItemBackgrounds() {
  ...
  for (int i = 0; i < m_presetLayout->count(); ++i) {
    ...
    if (BrushPresetItem *presetItem =
            dynamic_cast<BrushPresetItem *>(item->widget())) {
      presetItem->setShowBackgrounds(
          m_showBackgrounds);  // Synchronize and redraw
    }
  }
}
