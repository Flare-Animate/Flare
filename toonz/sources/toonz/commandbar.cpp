...
void CommandBar::fillToolbar(CommandBar *toolbar, CommandBarType barType,
  ...
  if (barType == CommandBarType::Quick) {
    personalPath =
        FlareFolder::getMyModuleDir() + TFilePath("quicktoolbar.xml");
  } else if (barType == CommandBarType::Main) {
    personalPath = FlareFolder::getMyModuleDir() + TFilePath("maintoolbar.xml");
  } else if (!barId.isEmpty()) {
    personalPath = FlareFolder::getMyModuleDir() + TFilePath("commandbars") +
                   TFilePath("commandbar_" + barId + ".xml");
    if (!TSystem::doesExistFileOrLevel(personalPath))
      personalPath =
          FlareFolder::getMyModuleDir() + TFilePath("commandbar.xml");
  } else {
  ...
  if (!fileFound) {
    if (barType == CommandBarType::Quick) {
      personalPath =
          FlareFolder::getTemplateModuleDir() + TFilePath("quicktoolbar.xml");
    } else if (barType == CommandBarType::Main) {
      personalPath =
          FlareFolder::getTemplateModuleDir() + TFilePath("maintoolbar.xml");
    } else {
      personalPath =
...
void CommandBar::onCloseButtonPressed() {
  ...
  TFilePath commandbarFile = FlareFolder::getMyModuleDir() +
